...
function openBulkIngredientReview(focusIndex=-1){
  ...
  const unresolved=ingredients.length-matched;
  showModal(`<div class="bulk-review-header"><div><div class="eyebrow">Ingredient review</div><h2>${esc(recipeDraft.name||'Recipe')}</h2></div><button class="ghost compact" onclick="renderRecipeBuilderModal()">Back to recipe</button></div>
    <div class="notice"><strong>${matched}/${ingredients.length} linked</strong>${unresolved?` · ${unresolved} still need review`:''}<br>Edit the cleaned food names in one list. Parentheses, cutting instructions and punctuation are excluded from searches.</div>
    <div class="bulk-review-toolbar"><button class="secondary compact" onclick="cleanAllRecipeIngredientNames()">Clean all names</button><button class="secondary compact" onclick="autoMatchBulkIngredients()">Auto-match clear results</button><button class="ghost compact" onclick="findUnmatchedIngredientsOnline()">Find unmatched online</button></div>
...
function unlinkRecipeIngredientData(ing){ing.ingredientId='';ing.matchStatus=ingredientChoiceOptions(ing).length>1&&!ing.selectedAlternative?'choose-alternative':'unmatched';ing.matchConfidence=0;ing.grams='';ing.kcal=ing.protein=ing.carbs=ing.fat=ing.fibre=ing.sodiumMg='';}
