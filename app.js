...
function openRecipeView(id){
  ...
  normaliseRecipeRecord(recipe);activeRecipeViewId=id;renderRecipeViewModal();
}
function renderRecipeViewModal(){
  ...
  showModal(`<div class="recipe-reader">
    <header class="recipe-reader-header">
      <button class="ghost compact" onclick="closeModal()">Close</button>
      <div><div class="tiny muted">${esc(recipe.category||'Recipe')}</div><h2>${esc(recipe.name)}</h2></div>
      <button class="ghost compact" onclick="closeModal();openRecipeBuilder('${esc(recipe.id)}')">Edit</button>
    </header>
    <div class="recipe-reader-body">
      ${recipe.imageUrl?`<img class="recipe-reader-image" src="${esc(recipe.imageUrl)}" alt="" referrerpolicy="no-referrer" onerror="this.remove()">`:''}
      <section class="card flat recipe-overview">
        ...
        <div class="tag-list" style="margin-top:10px"><span class="pill ${confidence.label==='High'?'good':confidence.label==='Medium'?'warn':'gray'}">${esc(confidence.label)} nutrition</span>${recipe.prepTime?`<span class="pill gray">Prep ${esc(recipe.prepTime)}</span>`:''}${recipe.cookTime?`<span class="pill gray">Cook ${esc(recipe.cookTime)}</span>`:''}</div>
        ${flags.length?`<div class="notice warn compact-notice" style="margin-top:8px">${flags.map(esc).join(' · ')}</div>`:''}
        <div class="button-row" style="margin-top:10px">
          <button class="secondary compact" onclick="closeModal();openAddFood('${esc(recipe.id)}')" ${toNum(recipe.totalKcal)<=0?'disabled':''}>Log serving</button>
          ${cookTimers.map(t=>`<button class="ghost compact" onclick="startRecipeTimer(event,'${esc(recipe.name)}',${t.seconds},'${esc(t.label)}')">⏱ ${esc(t.label)}</button>`).join('')}
          ${recipe.sourceUrl?`<button class="ghost compact" onclick="openExternalSource('${esc(recipe.sourceUrl)}')">Source</button>`:''}
        </div>
      </section>
      <section class="card flat cooking-progress-card">
        ...
        <button class="ghost compact" style="margin-top:8px" onclick="resetCookingProgress('${esc(recipe.id)}')">Reset checklist</button>
      </section>
...
function returnToRecipeBuilderFromMatch(){renderRecipeBuilderModal();}
...
function openIngredientEditor(id='',returnIndex='') {
  ...
  const closeAction=returnIndex!==''?`openIngredientMatch(${Number(returnIndex)},false)`:'closeModal()';
...
const RECIPE_UNITS=['kilograms','kilogram','kg','grams','gram','g','milligrams','milligram','mg','litres','litre','liters','liter','l','millilitres','millilitre','milliliters','milliliter','ml','fluid ounces','fluid ounce','fl oz','cups','cup','tablespoons','tablespoon','tbsp','teaspoons','teaspoon','tsp','ounces','ounce','oz','pounds','pound','lb','cloves','clove','cans','can','tins','tin','slices','slice','scoops','scoop','pieces','piece','units','unit','each','medium','small','large','bunches','bunch','handfuls','handful','pinches','pinch','sticks','stick','sprigs','sprig','heads','head','stalks','stalk','inches','inch','limes','lime','lemons','lemon'];
...
function cleanIngredientSearchText(value){
  let text=String(value||'').replace(/&amp;/gi,' and ').replace(/\s+/g,' ').trim();
  PREPARATION_PHRASE_PATTERNS.forEach(pattern=>{text=text.replace(pattern,' ');});
  text=text.replace(/(?:,|;)\s*(?:finely|roughly|coarsely|thinly|thickly|freshly|chopped|diced|sliced|minced|grated|shredded|crushed|peeled|seeded|deseeded|trimmed|divided|to taste|for garnish(?:ing)?|to serve).*$/i,' ');
...
function normaliseRecipeRecord(recipe){
  ...
  recipe.ingredients.forEach(ing=>{
    ...
    ing.matchConfidence=toNum(ing.matchConfidence);ing.fibre=toNum(ing.fibre);ing.sodiumMg=toNum(ing.sodiumMg);ing.notes ||= '';ing.updatedAt ||= recipe.updatedAt||nowISO();
  });
  return recipe;
}
function normaliseFoodName(value,{loose=false}={}){
  const words=cleanIngredientSearchText(value).toLowerCase().replace(/&amp;/g,' and ').replace(/\byoghurt\b/g,' yogurt ').replace(/\blow[-\s]?sodium\b/g,' ').replace(/\bjuice\s+of\b/g,' juice ').replace(/[^a-z0-9\s]/g,' ').split(/\s+/).filter(Boolean).filter(w=>!PREP_WORDS.has(w));
  return words.filter(w=>!loose||!FOOD_DESCRIPTOR_WORDS.has(w)).map(w=>w.endsWith('ies')?w.slice(0,-3)+'y':w.endsWith('oes')?w.slice(0,-2):w.endsWith('s')&&w.length>4?w.slice(0,-1):w).join(' ').trim();
}
function referenceFoodQuery(value){
  let q=normaliseFoodName(value,{loose:true});
  const replacements=[[/\bsoy sauce\b.*/,'soy sauce'],[/\bcucumber\b.*/,'cucumber raw'],[/\bsalt\b.*/,'salt'],[/\brice vinegar\b.*/,'rice vinegar'],[/\bsesame oil\b.*/,'sesame oil'],[/\bchili paste\b.*/,'chili paste'],[/\bchilli paste\b.*/,'chili paste'],[/\blime\b.*/,'lime juice']];
  replacements.forEach(([r,v])=>{if(r.test(q))q=v;});return q||String(value||'').trim();
}
...
function canonicalUnit(unit){
  ...
  const map={kilograms:'kg',kilogram:'kg',grams:'g',gram:'g',milligrams:'mg',milligram:'mg',litres:'l',litre:'l',liters:'l',liter:'l',millilitres:'ml',millilitre:'ml',milliliters:'ml',milliliter:'ml',cups:'cup',tablespoon:'tbsp',tablespoons:'tbsp',teaspoon:'tsp',teaspoons:'tsp','fluid ounce':'fl oz','fluid ounces':'fl oz',cloves:'clove',cans:'can',tins:'can',tin:'can',slices:'slice',scoops:'scoop',pounds:'lb',pound:'lb',lbs:'lb',ounces:'oz',ounce:'oz',pieces:'each',piece:'each',units:'each',unit:'each',bunches:'bunch',handfuls:'handful',pinches:'pinch',sticks:'stick',sprigs:'sprig',heads:'head',stalks:'stalk',inches:'inch',limes:'lime',lemons:'lemon'};
  return map[u]||u;
}
...
function amountToGrams(amount,unit,item){
  ...
  const density=inferredDensity(item);
  if(u==='ml')return density?a*density:'';if(u==='l')return density?a*1000*density:'';
  if(STANDARD_VOLUME_ML[u])return density?a*STANDARD_VOLUME_ML[u]*density:'';
...
function recalculateRecipeIngredient(ing){
  ...
  normaliseIngredientRecord(item);if(ing.amount===''||toNum(ing.amount)<=0){ing.grams='';ing.matchStatus='missing-amount';ing.kcal=ing.protein=ing.carbs=ing.fat=ing.fibre=ing.sodiumMg='';return;}
  const baseGrams=ingredientBaseGrams(item),grams=amountToGrams(ing.amount,ing.unit,item);ing.grams=grams;
  ...
  if(ing.matchStatus==='needs-measure'||ing.matchStatus==='missing-amount')ing.matchStatus=item.confirmed?'confirmed':'likely';ing.updatedAt=nowISO();
}
function foodMatchScore(query,item){
  ...
  for(const name of names){
    ...
    else if(name.includes(strict)||strict.includes(name)||name.includes(loose)||loose.includes(name))best=Math.max(best,86);
    else{const qa=new Set((loose||strict).split(' ')),na=new Set(name.split(' '));const intersect=[...qa].filter(x=>na.has(x)).length;const union=new Set([...qa,...na]).size;best=Math.max(best,union?intersect/union*78:0);}
  }
  return Math.round(best);
}
function localFoodMatches(query,limit=8){return state.ingredientCache.map(item=>({item,score:foodMatchScore(query,item)})).filter(x=>x.score>=25).sort((a,b)=>b.score-a.score||Number(Boolean(b.item.confirmed))-Number(Boolean(a.item.confirmed))).slice(0,limit);}
...
function autoMatchRecipeIngredients(recipe){
  normaliseRecipeRecord(recipe);recipe.ingredients.forEach(ing=>{
    ...
    if(matches[0]&&matches[0].score>=52)applyIngredientMatch(ing,matches[0].item,matches[0].score>=92&&matches[0].item.confirmed?'confirmed':'likely',matches[0].score);
  });return recipe;
}
...
async function findUnmatchedIngredientsOnline(){
  ...
  try{
    ...
    saveState();renderRecipeBuilderModal();showToast(`${matched} online ingredient match${matched===1?'':'es'} added${targets.length>15?' · first 15 checked':''}`,5000);
  }catch(error){renderRecipeBuilderModal();showToast(error.message||'Online matching failed',5000);}finally{recipeImportBusy=false;}
}
function recipeNutritionConfidence(recipe){
  ...
  const calculable=recipe.ingredients.filter(i=>Number.isFinite(Number(i.kcal))&&i.kcal!==''&&!['unmatched','needs-measure','missing-amount'].includes(i.matchStatus)).length;
...
function recipeIngredientEditorHtml(ing,index){
  const status=ing.matchStatus||'unmatched',statusClass=status==='confirmed'?'good':status==='likely'?'warn':['needs-measure','missing-amount','choose-alternative'].includes(status)?'warn':'danger';
  const statusText=status==='needs-measure'?'Matched · unit conversion needed':status==='missing-amount'?'Matched · amount needed':status==='choose-alternative'?'Choose ingredient option':status==='unmatched'?'Unmatched':status[0].toUpperCase()+status.slice(1);
  const alternatives=ingredientChoiceOptions(ing);
  return `<div class="row recipe-ingredient-row" id="recipeIngredientRow-${index}">
    ...
    <div class="button-row"><button type="button" class="secondary compact" onclick="openIngredientMatch(${index})">${ing.ingredientId?'Change match':alternatives.length>1&&!ing.selectedAlternative?'Choose & match':'Match ingredient'}</button>${ing.ingredientId?`<button type="button" class="ghost compact" onclick="unlinkRecipeIngredient(${index})">Unlink</button>`:''}</div>
    ...
    ${ing.matchStatus==='needs-measure'?'<div class="tiny muted">The food was matched, but this household unit needs a gram conversion.</div>':ing.matchStatus==='missing-amount'?'<div class="tiny muted">The food was matched. Enter an amount only if you want it included in nutrition totals.</div>':ing.matchStatus==='choose-alternative'?'<div class="tiny muted">This recipe offers alternatives. Choose which ingredient you are actually using before matching nutrition.</div>':''}
  </div>`;
}
function unlinkRecipeIngredient(index){const ing=recipeDraft?.ingredients?.[index];if(!ing)return;ing.ingredientId='';ing.matchStatus=ingredientChoiceOptions(ing).length>1&&!ing.selectedAlternative?'choose-alternative':'unmatched';ing.matchConfidence=0;ing.grams='';ing.kcal=ing.protein=ing.carbs=ing.fat=ing.fibre=ing.sodiumMg='';renderRecipeBuilderModal();}
function openIngredientMatch(index,preservePosition=true){
  const ing=recipeDraft?.ingredients?.[index];if(!ing)return;if(preservePosition)rememberRecipeEditorPosition(index);
...
function selectIngredientAlternative(index,optionIndex){
  ...
  ing.alternativeOptions=options;ing.selectedAlternative=choice;ing.name=choice;ing.matchStatus='unmatched';ing.updatedAt=nowISO();openIngredientMatch(index,false);
}
...
function recipeFromImportedData(data,sourceType='Website'){
  const recipe={id:'',name:data.name||'Imported recipe',category:'Imported',servings:toNum(data.servings)||1,totalKcal:0,protein:0,carbs:0,fat:0,fibre:0,sodiumMg:0,rflFriendly:'No',favourite:false,noBellPeppers:Boolean(state.settings.noBellPeppers),noShellfish:Boolean(state.settings.noShellfish),instructions:data.instructions||'',notes:'',ingredients:(data.ingredients||[]).map(line=>{const parsed=parseIngredientLine(line),alternatives=ingredientAlternativeNames(parsed.name);return {id:uid('ring'),ingredientId:'',name:parsed.name,sourceLine:line,amount:parsed.amount,unit:parsed.unit||'',grams:'',matchStatus:alternatives.length>1?'choose-alternative':'unmatched',matchConfidence:0,alternativeOptions:alternatives,selectedAlternative:'',kcal:'',protein:'',carbs:'',fat:'',fibre:'',sodiumMg:'',notes:parsed.notes||'',updatedAt:nowISO()};}),calculateFromIngredients:true,sourceType,sourceName:data.sourceName||'',sourceUrl:data.sourceUrl||'',sourceAuthor:data.author||'',sourceBook:data.bookTitle||'',importedAt:nowISO(),prepTime:data.prepTime||'',cookTime:data.cookTime||'',totalTime:data.totalTime||'',yieldText:data.yieldText||'',finishedWeightG:'',nutritionConfidence:'Low',importStatus:'Needs review',tags:Array.isArray(data.tags)?data.tags:[],imageUrl:data.imageUrl||'',sourceNutrition:data.nutrition||{},updatedAt:nowISO()};normaliseRecipeRecord(recipe);return recipe;
}
