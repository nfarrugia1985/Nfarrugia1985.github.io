...
/* Lift & Cut 2.5.1 — stability, migration, sync and data protection.
...
const LC_V251 = Object.freeze({
  ...
  MAX_SNAPSHOTS:12,
  SYNC_COLLECTIONS:['programs','exercises','alternatives','ingredientCache','recipes','workoutSessions','workoutLogs','foodLog','bodyMetrics','dailyCheckins','dietPhases','nutritionDays','targetAdjustments','weeklyReviews','rflProfiles','rflDailyLogs','rflEvents','cardioLogs','progressPhotos'],
...
loadState=async function(){
  ...
  if(source==='starter'&&!bootNotice)bootNotice='Lift & Cut 2.5.1 data protection is active.';
  return next;
};
...
pullCloud=async function(replace=false){
  ...
  try{state.meta.syncStatus='pending';updateSyncIndicator();const result=await jsonpRequest({action:'pull',key:state.settings.syncKey,clientId:state.meta.clientId});if(!result?.ok||!result.state)throw new Error(result?.error||'No cloud state available');
    const audit=LiftCutSafety.validateState(result.state,{maxSchema:LC_V251.SCHEMA});if(!audit.ok)throw new Error(`Cloud state failed validation: ${audit.errors[0]}`);const raw=JSON.stringify(result.state),hash=await LiftCutSafety.sha256Text(raw);if(result.stateHash&&hash!==result.stateHash)throw new Error('Cloud state checksum did not match. No phone data was changed.');
    const revision=toNum(result.revision),stateHash=String(result.stateHash||hash);if(replace){pendingCloudState={state:result.state,revision,stateHash,audit};const current=LiftCutSafety.stateSummary(state),cloud=LiftCutSafety.stateSummary(result.state);showModal(`<div class="card-title"><span>Review cloud replacement</span><button class="ghost compact" onclick="cancelCloudReplacement()">Cancel</button></div><div class="compare-grid"><div><strong>Phone now</strong><div class="small">${esc(backupSummaryLine(current))}</div></div><div><strong>Cloud copy</strong><div class="small">${esc(backupSummaryLine(cloud))}</div></div></div><div class="notice danger">This replaces the phone database. A rollback snapshot is created first.</div><label>Type RESTORE CLOUD<input id="cloudReplacePhrase"></label><button class="danger" onclick="confirmCloudReplacement()">Replace phone copy</button>`);return;}
    await v251ApplyCloud(result.state,revision,stateHash,'merge');
