...
function openIngredientMatch(index,preservePosition=true,returnView=''){
  ...
  showModal(`<div class="card-title"><span>Match ingredient</span><button class="ghost compact" onclick="returnToRecipeBuilderFromMatch()">Back</button></div>
    ...
    <div class="button-row" style="margin-top:8px"><button onclick="searchReferenceFoods(${index})" ${alternatives.length>1&&!selected?'disabled':''}>Search live USDA</button><button class="ghost" onclick="openIngredientEditor('',${index})">Add manually</button></div>
