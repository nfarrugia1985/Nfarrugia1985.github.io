...
function saveDraft() {
  if (workoutDraft) localStorage.setItem(DRAFT_KEY, JSON.stringify(workoutDraft));
...
function startSession(programId, sessionId) {
  ...
  if (workoutDraft && workoutDraft.exercises?.some(e => e.sets.some(s => s.weightKg || s.reps))) {
    if (!confirm('Replace the current unsaved workout draft?')) return;
  }
...
function buildWorkoutDraft(program, session) {
  return {
    ...
    exercises:[...(session.exercises || [])].sort((a,b)=>toNum(a.order)-toNum(b.order)).map(slot => {
      const previous = lastExerciseSets(slot.name).filter(x=>!x.warmup);
      const sets = [];
      for (let i=0;i<Math.max(1,toNum(slot.sets));i++) {
        ...
        sets.push({
          ...
          weightKg:prior?.weightKg ?? '', reps:prior?.reps ?? '',
          rir:prior?.rir ?? '', done:false
        });
      }
      return {
...
function ensureWorkoutDraft() {
  ...
  if (stored?.programId && stored?.sessionId) {
    workoutDraft = stored;
    return workoutDraft;
  }
  const {program, session} = selectedSession();
  if (program && session) workoutDraft = buildWorkoutDraft(program, session);
  saveDraft();
  return workoutDraft;
}
...
function toggleSetDone(exIndex, setIndex) {
  ...
  set.done = !set.done;
  saveDraft();
  if (set.done && setIndex < ex.sets.length - 1) startRestTimer(ex.restSec);
  render();
}
function addDraftSet(exIndex) {
  ...
  ex.sets.push({
    ...
    weightKg:prior.weightKg ?? '', reps:prior.reps ?? '', rir:prior.rir ?? '', done:false
  });
  saveDraft(); render();
}
...
function resetWorkoutDraft() {
  if (!confirm('Discard the current workout draft?')) return;
  const {program,session} = selectedSession();
  workoutDraft = program && session ? buildWorkoutDraft(program,session) : null;
  saveDraft(); render(); showToast('Draft reset');
}
function renderTrain() {
  const draft = ensureWorkoutDraft();
  const options = state.programs.filter(p=>p.active!==false).flatMap(p => (p.sessions||[]).map(s => ({
    value:sessionKey(p.id,s.id), label:`${p.name} — ${s.name}`
  })));
  const recent = [...state.workoutSessions].sort((a,b)=>String(b.date+(b.startTime||'')).localeCompare(String(a.date+(a.startTime||'')))).slice(0,8);
  if (!draft) return `<div class="empty">Create a program session first.</div>`;
  return `<div class="grid">
    <div class="card highlight">
      <div class="card-title"><span>Workout draft</span><span class="pill ${state.settings.dietMode === 'RFL / PSMF' ? 'warn':''}">${esc(state.settings.dietMode)}</span></div>
      <label>Session<select onchange="changeTrainSession(this.value)">
        ${options.map(o=>`<option value="${esc(o.value)}" ${o.value===sessionKey(draft.programId,draft.sessionId)?'selected':''}>${esc(o.label)}</option>`).join('')}
      </select></label>
      <div class="split" style="margin-top:10px">
        <label>Date<input type="date" value="${esc(draft.date)}" onchange="updateDraftField('date',this.value)"></label>
        <label>Body weight kg<input type="number" step="0.1" inputmode="decimal" value="${esc(draft.bodyWeightKg)}" onchange="updateDraftField('bodyWeightKg',this.value)"></label>
      </div>
      <label style="margin-top:10px">Session notes<input value="${esc(draft.notes || '')}" onchange="updateDraftField('notes',this.value)" placeholder="Energy, pain, equipment changes..."></label>
      <div class="small muted" style="margin-top:8px">Tap the set number to mark a warm-up set. Completed work sets can start the rest timer automatically.</div>
    </div>
    ...
    <div class="card">
      <div class="button-row">
        <button onclick="saveWorkout()">Save completed workout</button>
        <button class="ghost" onclick="resetWorkoutDraft()">Reset draft</button>
      </div>
      <div class="small muted" style="margin-top:8px">Only sets with repetitions greater than zero are saved. Warm-up sets remain visible in history but are excluded from work-set totals.</div>
    </div>

    <div class="card">
      <div class="card-title"><span>Recent workouts</span><span class="pill gray">${state.workoutSessions.length}</span></div>
      ${recent.length ? `<div class="list">${recent.map(s=>renderWorkoutHistoryRow(s)).join('')}</div>` : `<div class="empty">No saved workouts yet.</div>`}
    </div>
  </div>`;
}
function renderWorkoutExercise(ex, exIndex) {
  const last = lastExerciseSets(ex.name).filter(x=>!x.warmup);
  const best = workoutBest(ex.name);
  const lastText = last.length ? last.map(x=>`${round(x.weightKg,1)}×${round(x.reps,0)}`).join(', ') : 'No previous work sets';
  return `<div class="card">
    <div class="card-title">
      <div><span>${esc(ex.name)}</span><div class="small muted">${esc(ex.targetSets)} × ${esc(ex.targetReps)} · target RIR ${esc(ex.targetRir)}</div></div>
      <button class="secondary compact" onclick="startRestTimer(${toNum(ex.restSec)||120})">⏱ ${toNum(ex.restSec)||120}s</button>
    </div>
    <div class="notice"><strong>Last:</strong> ${esc(lastText)}${best?.estimated1RM ? ` · best e1RM ${round(best.estimated1RM,1)} kg` : ''}</div>
    <div class="set-row small muted" style="border-bottom:0;padding-bottom:0">
      <span></span><span style="text-align:center">kg</span><span style="text-align:center">reps</span><span style="text-align:center">RIR</span><span></span>
    </div>
    ${ex.sets.map((set,setIndex)=>`<div class="set-row">
      <button type="button" class="ghost compact set-number" onclick="toggleWarmup(${exIndex},${setIndex})">${set.warmup ? 'W' : setIndex+1}</button>
      <input aria-label="Weight kg" type="number" step="0.25" inputmode="decimal" value="${esc(set.weightKg)}" onchange="updateDraftSet(${exIndex},${setIndex},'weightKg',this.value)">
      <input aria-label="Repetitions" type="number" step="1" inputmode="numeric" value="${esc(set.reps)}" onchange="updateDraftSet(${exIndex},${setIndex},'reps',this.value)">
      <input aria-label="Repetitions in reserve" type="number" step="0.5" inputmode="decimal" value="${esc(set.rir)}" onchange="updateDraftSet(${exIndex},${setIndex},'rir',this.value)">
      <button type="button" class="set-done ${set.done?'done':''}" onclick="toggleSetDone(${exIndex},${setIndex})">${set.done?'✓':'○'}</button>
    </div>`).join('')}
    <div class="button-row" style="margin-top:8px">
      <button class="secondary compact" onclick="addDraftSet(${exIndex})">＋ Set</button>
      ${ex.sets.length>1 ? `<button class="ghost compact" onclick="removeDraftSet(${exIndex},${ex.sets.length-1})">Remove last</button>`:''}
    </div>
    <label style="margin-top:8px">Exercise notes<input value="${esc(ex.notes || '')}" onchange="updateDraftExerciseNote(${exIndex},this.value)" placeholder="Technique, pain, tempo..."></label>
  </div>`;
}
...
function saveWorkout() {
  ...
  draft.exercises.forEach(ex => {
    ex.sets.forEach((set,index) => {
      ...
      setRows.push({
        ...
        weightKg:weight,
        reps,
        rir:numOrBlank(set.rir),
        restSec:toNum(ex.restSec),
        volumeKg:weight * reps,
        estimated1RM:calculateE1RM(weight,reps),
        notes:ex.notes || '',
        updatedAt:nowISO()
      });
    });
  ...
  const priorBests = {};
  [...new Set(setRows.map(x=>x.exercise))].forEach(name => priorBests[name] = toNum(workoutBest(name)?.estimated1RM));
  const end = new Date();
  const start = new Date(draft.startedAt || end);
  const sessionLog = {
    id:uid('workout'),
    date:draft.date || localDateISO(),
    programId:draft.programId,
    ...
    startTime:String(draft.startedAt || '').slice(11,16),
    endTime:localDateTimeISO(end).slice(11,16),
    durationMin:Math.max(1,Math.round((end-start)/60000)),
    bodyWeightKg:numOrBlank(draft.bodyWeightKg),
    notes:draft.notes || '',
    completed:true,
    updatedAt:nowISO()
  };
  setRows.forEach(x=>x.sessionLogId=sessionLog.id);
  state.workoutSessions.push(sessionLog);
  state.workoutLogs.push(...setRows);
  const prs = [...new Set(setRows.filter(x=>!x.warmup && x.estimated1RM > (priorBests[x.exercise]||0)).map(x=>x.exercise))];
  saveState();
  workoutDraft = null;
  ...
  render();
  showToast(prs.length ? `Workout saved · new best: ${prs.slice(0,2).join(', ')}${prs.length>2?'…':''}` : 'Workout saved');
}
function renderWorkoutHistoryRow(session) {
  const sets = state.workoutLogs.filter(x=>x.sessionLogId===session.id);
  const workSets = sets.filter(x=>!x.warmup);
  const volume = workSets.reduce((s,x)=>s+toNum(x.volumeKg),0);
  return `<div class="row">
    <div class="row-head"><div><strong>${esc(session.sessionName)}</strong><div class="small muted">${esc(session.programName || session.programId)} · ${workSets.length} work sets · ${round(volume/1000,1)} t</div></div><span class="pill gray">${esc(formatDate(session.date))}</span></div>
    <div class="button-row">
      <button class="secondary compact" onclick="viewWorkout('${esc(session.id)}')">View</button>
      <button class="ghost compact" onclick="repeatWorkout('${esc(session.id)}')">Repeat</button>
      <button class="danger compact" onclick="deleteWorkoutSession('${esc(session.id)}')">Delete</button>
    </div>
  </div>`;
}
function viewWorkout(id) {
  const session = state.workoutSessions.find(x=>x.id===id);
  if (!session) return;
  const rows = state.workoutLogs.filter(x=>x.sessionLogId===id).sort((a,b)=>a.exercise.localeCompare(b.exercise)||toNum(a.setNo)-toNum(b.setNo));
  const grouped = rows.reduce((m,x)=>{(m[x.exercise]||=[]).push(x);return m;},{});
  showModal(`<div class="card-title"><span>${esc(session.sessionName)}</span><button class="ghost compact" onclick="closeModal()">Close</button></div>
    <div class="small muted">${esc(session.date)} · ${round(session.durationMin,0)} min · ${esc(session.notes||'')}</div>
    <div class="list" style="margin-top:10px">${Object.entries(grouped).map(([name,sets])=>`<div class="row"><strong>${esc(name)}</strong>
      <table class="history-table"><thead><tr><th>Set</th><th>kg</th><th>Reps</th><th>RIR</th><th>e1RM</th></tr></thead><tbody>
      ${sets.map(x=>`<tr><td>${x.warmup?'W':esc(x.setNo)}</td><td>${round(x.weightKg,1)}</td><td>${round(x.reps,0)}</td><td>${esc(x.rir)}</td><td>${x.estimated1RM?round(x.estimated1RM,1):'—'}</td></tr>`).join('')}
      </tbody></table></div>`).join('')}</div>`);
}
function repeatWorkout(id) {
  const past = state.workoutSessions.find(x=>x.id===id);
  if (!past) return;
...
function deleteWorkoutSession(id) {
  ...
  state.workoutSessions = state.workoutSessions.filter(x=>x.id!==id);
  state.workoutLogs = state.workoutLogs.filter(x=>x.sessionLogId!==id);
  saveState(); render(); showToast('Workout deleted');
}
function startRestTimer(seconds) {
  restTimerSeconds = Math.max(1,toNum(seconds)||120);
  clearInterval(restTimerInterval);
  updateRestTimerDisplay();
  document.getElementById('restTimer').hidden = false;
  restTimerInterval = setInterval(()=>{
    ...
    if (restTimerSeconds <= 0) {
      ...
      setTimeout(stopRestTimer,1200);
    }
  },1000);
}
function updateRestTimerDisplay() {
  const m = Math.floor(Math.max(0,restTimerSeconds)/60);
  const s = Math.max(0,restTimerSeconds)%60;
  const el = document.getElementById('restTimerValue');
  if (el) el.textContent = `${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`;
}
function adjustRestTimer(seconds) {
  restTimerSeconds = Math.max(0,restTimerSeconds + seconds);
  updateRestTimerDisplay();
}
...
function renderSettings() {
  ...
  return `<div class="grid">
    ...
    <details class="card">
      ...
      <label style="margin-top:10px">Default rest timer seconds<input type="number" step="15" value="${esc(state.settings.restTimerDefaultSec)}" onchange="updateSetting('restTimerDefaultSec',this.value)"></label>
...
function updateSetting(key,value) {
  const numeric=['age','heightCm','targetWeightKg','normalCalorieTarget','normalProteinTarget','normalCarbTarget','normalFatTarget',
    'rflCalorieTarget','rflProteinTarget','rflCarbTarget','rflFatTarget','stepsTarget','sleepTargetHrs','waterTargetMl','restTimerDefaultSec'];
