...
function migrateState(input) {
  ...
  const arrays = ['programs','exercises','alternatives','ingredientCache','recipes','workoutSessions',
    ...
    'nutritionDays','targetAdjustments','weeklyReviews','rflProfiles','rflDailyLogs','rflEvents','savedFoods','mealTemplates'];
  ...
  // Merge newly shipped built-in programs into existing installations without overwriting user programs.
  const existingProgramIds = new Set(next.programs.map(item => item.id));
  DEFAULT_STATE.programs.forEach(seed => {
    if (seed.builtIn === true && !existingProgramIds.has(seed.id)) next.programs.push(deepClone(seed));
  });
  ...
  next.programs.forEach((p, pi) => {
    ...
    const shippedProgram = ['UL4','RFL2','GVS_EXTRA','MLM6_MOD'].includes(p.id);
    if (p.builtIn === undefined) p.builtIn = shippedProgram;
    p.sourceType ||= shippedProgram ? 'Built-in' : 'Custom';
  ...
  ['exercises','alternatives','ingredientCache','recipes','workoutSessions','workoutLogs',
    ...
    'nutritionDays','targetAdjustments','weeklyReviews','rflProfiles','rflDailyLogs','rflEvents','savedFoods','mealTemplates'].forEach(key => {
...
function upgradeStoredRecipeConversions(){
  ...
  if(changed)saveState({touch:false,autoSync:false});
}
...
function renderDashboard() {
  ...
  return `<div class="grid">
    ${renderInstallBanner()}
    ${typeof renderRflDashboardCard === 'function' ? renderRflDashboardCard() : ''}
      ...
      <div class="grid">
        <div class="card">
          ...
          ${phase ? `<div class="stack">
            ...
            <button class="secondary" onclick="${phase.mode==='RFL / PSMF'?'openRflPage()':'openCoachPage()'}">${phase.mode==='RFL / PSMF'?'Open RFL mode':'Open phase coaching'}</button>
  ...
  return `<div class="grid">
    <div class="card">
      ...
      <div class="button-row" style="margin-top:10px">
        ...
        ${program && !isBuiltInProgram(program) ? `<button class="danger compact" onclick="deleteCustomProgram('${esc(program.id)}')">Delete program</button>` : ''}
      </div>
...
function saveNewProgram(event) {
  ...
  const program = {
    ...
    active: true,
    builtIn: false,
    sourceType: clone ? 'Cloned custom' : 'Custom',
    updatedAt: nowISO(),
    sessions: []
  };
  if (clone && source) {
...
function isBuiltInProgram(program) {
  if (!program) return false;
  return program.builtIn === true || ['UL4','RFL2','GVS_EXTRA','MLM6_MOD'].includes(program.id);
}
function deleteCustomProgram(programId) {
  const program = state.programs.find(item => item.id === programId);
  if (!program) return showToast('Program not found');
  if (isBuiltInProgram(program)) return showToast('Built-in programs are protected');
  const historyCount = (state.workoutSessions || []).filter(row => row.programId === programId).length;
  const sessionCount = (program.sessions || []).length;
  const message = `Delete “${program.name}” and its ${sessionCount} program session${sessionCount===1?'':'s'}?\n\n${historyCount ? `${historyCount} completed workout record${historyCount===1?'':'s'} will stay in History.` : 'No completed workout history will be removed.'}`;
  if (!confirm(message)) return;
  recordDeletion('programs',programId,'custom program deleted');
  const wasActive = state.settings.activeProgram === programId;
  state.programs = state.programs.filter(item => item.id !== programId);
  (state.rflProfiles || []).forEach(profile => {
    if (profile.previousProgramId === programId) { profile.previousProgramId = 'UL4'; profile.updatedAt = nowISO(); }
  });
  if (workoutDraft?.programId === programId) { workoutDraft = null; saveDraft(); }
  if (wasActive) {
    const rflActive = typeof activeRflPhase === 'function' && activeRflPhase();
    const fallback = state.programs.find(item => item.id === (rflActive ? 'RFL2' : 'UL4')) || state.programs.find(item => item.active !== false);
    state.settings.activeProgram = fallback?.id || '';
    state.settings.lastSession = fallback ? sessionKey(fallback.id, fallback.sessions?.[0]?.id || '') : '';
  }
  saveState();
  closeModal();
  render();
  showToast('Custom program deleted; workout history preserved',4200);
}
...
function deleteExercise(id) {
  ...
  recordDeletion('exercises',id,'exercise deleted');
...
function deleteWorkoutSession(id) {
  ...
  recordDeletion('workoutSessions',id,'workout deleted');
  state.workoutLogs.filter(item=>item.sessionLogId===id).forEach(item=>recordDeletion('workoutLogs',item.id,'parent workout deleted'));
...
function deleteFood(id) {
  recordDeletion('foodLog',id,'food entry deleted');
    ...
    <form class="stack" onsubmit="saveRecipe(event)">
      ...
      <div class="card soft flat">
        ...
        <div class="button-row" style="margin-top:8px">
          ...
          <button type="button" class="secondary compact" onclick="findUnmatchedIngredientsOnline()">Live lookup remaining</button>
...
function deleteRecipe(id) {
  ...
  recordDeletion('recipes',id,'recipe deleted');
...
function autoMatchRecipeIngredients(recipe){
  normaliseRecipeRecord(recipe);recipe.ingredients.forEach(ing=>{
    ...
    else if(typeof lcUsda262Ready==='function'&&lcUsda262Ready()&&typeof lcUsda262Search==='function'){const food=lcUsda262Search(query,1)[0];if(food&&food.score>=850){let item=typeof lcUsda262FindSaved==='function'?lcUsda262FindSaved(food,'ingredientCache'):null;if(!item&&typeof lcUsda262SavedRecord==='function'){item=lcUsda262SavedRecord(food,{ingredient:true,alias:query});state.ingredientCache.push(item);}if(item)applyIngredientMatch(ing,item,'likely',Math.min(99,Math.round(food.score/12)));}}
  });return recipe;
}
...
async function findUnmatchedIngredientsOnline(){
  ...
  try{
    for(let n=0;n<Math.min(targets.length,15);n++){
      ...
      const q=referenceFoodQuery(ingredientMatchQuery(ing));const result=await jsonpRequest({action:'food-search',key:state.settings.syncKey,q,source:'usda'},22000);const foods=result?.foods||[];
...
function bulkIngredientRowHtml(ing,index){
  const status=ingredientReviewStatus(ing),alternatives=ingredientChoiceOptions(ing),query=ingredientMatchQuery(ing),matches=query?localFoodMatches(query,3):[],top=matches[0],usdaSuggestion=!top&&typeof lcUsda262BestBulkHtml==='function'?lcUsda262BestBulkHtml(ing,index):'';
  ...
  return `<section class="ingredient-review-row" id="ingredientReviewRow-${index}">
    ...
    ${linked?`<div class="ingredient-linked-summary"><div><span class="tiny muted">Matched to</span><strong>${esc(linked.name)}</strong>${linked.brand?`<small>${esc(linked.brand)}</small>`:''}${ing.measureNote?`<small>${esc(ing.measureNote)}</small>`:''}</div><div class="button-row"><button type="button" class="ghost compact" onclick="openIngredientMatch(${index},false,'bulk')">Change</button><button type="button" class="ghost compact" onclick="unlinkBulkRecipeIngredient(${index})">Unlink</button></div></div>`:
      ...
      usdaSuggestion||`<div class="button-row"><button type="button" class="secondary compact" onclick="openIngredientMatch(${index},false,'bulk')">Search food</button><button type="button" class="ghost compact" onclick="openIngredientEditor('',${index})">Add manually</button></div>`}
  </section>`;
}
function openBulkIngredientReview(focusIndex=-1){
  ...
  showModal(`<div class="bulk-review-header"><div><div class="eyebrow">Ingredient review</div><h2>${esc(recipeDraft.name||'Recipe')}</h2></div><button class="ghost compact" onclick="renderRecipeBuilderModal()">Back to recipe</button></div>
    ...
    <div class="bulk-review-toolbar"><button class="secondary compact" onclick="cleanAllRecipeIngredientNames()">Clean all names</button><button class="secondary compact" onclick="autoMatchBulkIngredients()">Auto-match all locally</button><button class="secondary compact" onclick="typeof lcUsda262MatchUnresolved==='function'?lcUsda262MatchUnresolved():showToast('USDA library is still loading')">Match from USDA library</button><button class="secondary compact" onclick="resolveBulkUnitConversions()">Resolve household units</button></div>
...
function openIngredientMatch(index,preservePosition=true,returnView=''){
  ...
  const alternatives=ingredientChoiceOptions(ing),selected=ing.selectedAlternative||'',query=ingredientMatchQuery(ing),matches=query?localFoodMatches(query,10):[],usdaHtml=query&&typeof lcUsda262RecipeHtml==='function'?lcUsda262RecipeHtml(index,query):'';
  showModal(`<div class="card-title"><span>Match ingredient</span><button class="ghost compact" onclick="returnToRecipeBuilderFromMatch()">Back</button></div>
    ...
    <div class="button-row" style="margin-top:8px"><button onclick="searchReferenceFoods(${index})" ${alternatives.length>1&&!selected?'disabled':''}>Search live USDA (optional)</button><button class="ghost" onclick="openIngredientEditor('',${index})">Add manually</button></div>
...
function saveStandaloneReferenceFood(i){const result=foodSearchResults[i];if(!result)return;state.ingredientCache.push({id:uid('ingredient'),name:result.name,brand:result.brand||'',foodState:result.foodState||'',servingDescription:'100 g',baseAmount:100,baseUnit:'g',densityGPerMl:'',kcal:toNum(result.kcal),protein:toNum(result.protein),carbs:toNum(result.carbs),fat:toNum(result.fat),fibre:toNum(result.fibre),sodiumMg:toNum(result.sodiumMg),aliases:[],commonMeasures:result.commonMeasures&&typeof result.commonMeasures==='object'?result.commonMeasures:{},source:result.source,sourceId:result.sourceId||'',sourceUrl:result.sourceUrl||'',confirmed:true,notes:String(result.source||'').startsWith('USDA')?'USDA FoodData Central generic/reference value. Use a package label when a specific branded product differs.':'Imported from reference search; verify current branded labels.',updatedAt:nowISO()});saveState();closeModal();render();showToast('Reference food saved');}
...
function deleteMetric(id) {
  ...
  recordDeletion('bodyMetrics',id,'measurement deleted');
...
function deleteDietPhase(id) {
  ...
  recordDeletion('dietPhases',id,'diet phase deleted');
  state.rflProfiles.filter(x=>x.phaseId===id).forEach(x=>recordDeletion('rflProfiles',x.id,'parent phase deleted'));
  state.rflDailyLogs.filter(x=>x.phaseId===id).forEach(x=>recordDeletion('rflDailyLogs',x.id,'parent phase deleted'));
  state.rflEvents.filter(x=>x.phaseId===id).forEach(x=>recordDeletion('rflEvents',x.id,'parent phase deleted'));
...
async function deleteProgressPhoto(id) {
  ...
  recordDeletion('progressPhotos',id,'progress photo deleted');
...
function renderSettings() {
  ...
  const syncState=state.meta.syncStatus||'local';
  const syncTone=syncState==='synced'?'good':['conflict','cloud-newer','verification-needed','error'].includes(syncState)?'danger':syncState==='dirty'?'warn':'gray';
  return `<div class="grid">
    <div class="card">
    ...
    <div class="card">
      <div class="card-title"><span>RFL mode</span><span class="pill ${state.settings.dietMode==='RFL / PSMF'?'warn':'gray'}">${state.settings.dietMode==='RFL / PSMF'?'Active':'Optional'}</span></div>
      <div class="notice warn">Use the guided RFL workspace to calculate the book-based category and protein range, reduce training volume, track the daily plan, and create an explicit exit phase. It does not prescribe medication or supplement doses.</div>
      <div class="split" style="margin-top:10px">
        <label>Current RFL calorie target<input type="number" value="${esc(state.settings.rflCalorieTarget)}" onchange="updateSetting('rflCalorieTarget',this.value)"></label>
        <label>Current RFL protein target g<input type="number" value="${esc(state.settings.rflProteinTarget)}" onchange="updateSetting('rflProteinTarget',this.value)"></label>
        <label>Vegetable servings goal<input type="number" min="1" max="12" value="${esc(state.settings.rflVegetableGoal||3)}" onchange="updateSetting('rflVegetableGoal',this.value)"></label>
        <label class="inline-check"><input type="checkbox" ${state.settings.rflRestoreProgramOnEnd!==false?'checked':''} onchange="updateSetting('rflRestoreProgramOnEnd',this.checked)"> Restore prior program after RFL</label>
      </div>
      <div class="button-row" style="margin-top:10px"><button onclick="openRflPage()">Open RFL workspace</button><button class="secondary" onclick="openRflSetup(typeof activeRflPhase==='function'&&activeRflPhase()?activeRflPhase().id:'')">${state.settings.dietMode==='RFL / PSMF'?'Review setup':'Set up phase'}</button></div>
    </div>
    ...
    <div class="card" id="syncSettingsCard">
      <div class="card-title"><span>Google Sheets sync</span><span class="pill ${syncTone}">${esc(syncStatusLabel())}</span></div>
      <div class="notice ${['conflict','cloud-newer','verification-needed','error'].includes(syncState)?'warn':''}">The app saves on this phone first. v2.5.3 distinguishes the last cloud revision actually merged from a newer revision merely observed, so a status check cannot authorise an unsafe overwrite.</div>
      ...
      <div class="small muted" style="margin-top:8px">Last successful sync: ${esc(syncAge)} · base revision ${toNum(state.meta.lastCloudRevision)} · latest seen ${toNum(state.meta.lastCloudRevisionSeen)}</div>
      ${state.meta.lastSyncError?`<div class="notice danger" style="margin-top:8px">${esc(state.meta.lastSyncError)}</div>`:''}
    </div>
    ...
    <div class="card">
      <div class="card-title"><span>Data safety and backup</span><span class="pill ${localSaveError?'danger':'good'}">${localSaveError?'Attention':'Protected'}</span></div>
      <div class="button-row">
        <button onclick="exportFullBackup()">Export verified backup</button>
        <button class="secondary" onclick="document.getElementById('backupImport').click()">Review backup file</button>
        <button class="secondary" onclick="openDataSafetyCenter()">Safety centre</button>
        <a class="button ghost" href="./Fitness_Database_Template.xlsx" download>Excel template</a>
      </div>
      <input id="backupImport" type="file" accept="application/json,.json" hidden onchange="importBackup(event)">
      <div class="small muted" style="margin-top:8px">Verified backups include the phone database, workout draft, cooking checklists and compressed progress photos. The private sync key is excluded. Restore always shows a preview first.</div>
      <hr class="divider">
      <div class="button-row">
        ...
        <button class="ghost compact" onclick="exportCSV('rflProfiles')">RFL profiles CSV</button>
        <button class="ghost compact" onclick="exportCSV('rflDailyLogs')">RFL daily log CSV</button>
        <button class="ghost compact" onclick="exportCSV('rflEvents')">RFL events CSV</button>
      </div>
    </div>
    ...
    <details class="card">
      ...
      <div class="small muted" style="margin-top:8px">Schema v${state.schemaVersion} · local revision ${toNum(state.meta.revision)} · sync ${esc(syncStatusLabel())} · client ${esc(state.meta.clientId)}</div>
    </details>
...
function applyEquipmentAdaptation(programId) {
  ...
  clone.builtIn=false; clone.sourceType='Equipment-matched'; clone.updatedAt=nowISO(); clone.sessions.forEach((session,si)=>{session.id=uid('session');session.order=si+1;session.updatedAt=nowISO();session.exercises.forEach((slot,xi)=>{const replacement=replacementMap.get(slot.id);slot.id=uid('slot');slot.order=xi+1;if(replacement){slot.notes=[slot.notes,`Equipment adaptation from ${slot.name}.`].filter(Boolean).join(' ');slot.name=replacement.name;slot.exerciseId=replacement.id;}slot.updatedAt=nowISO();});});
...
function updateSetting(key,value) {
  const numeric=['age','heightCm','targetWeightKg','normalCalorieTarget','normalProteinTarget','normalCarbTarget','normalFatTarget',
    'rflCalorieTarget','rflProteinTarget','rflCarbTarget','rflFatTarget','rflVegetableGoal','stepsTarget','sleepTargetHrs','waterTargetMl','restTimerDefaultSec','weightIncrementKg','dumbbellWeightIncrementKg','smallWeightIncrementKg'];
...
function openQuickAdd() {
  showModal(`<div class="card-title"><span>Quick add</span><button class="ghost compact" onclick="closeModal()">Close</button></div>
    <div class="grid">
      ...
      ${typeof activeRflPhase==='function' && activeRflPhase() ? `<button class="secondary" onclick="closeModal();openRflDailyLog('${localDateISO()}')">RFL daily check-in</button>` : ''}
...
document.addEventListener('visibilitychange',()=>{if(document.visibilityState==='hidden'){saveDraft();if(typeof scheduleSafetyMirror==='function')scheduleSafetyMirror(0);}else{updateRestTimerDisplay();refreshWorkoutClock();}});
