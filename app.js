...
function render() {
  ...
  if (quickAdd) quickAdd.hidden = page === 'train' && (state.settings.trainView || 'workout') === 'workout';
  updateSyncIndicator();
...
function ensureWorkoutDraft() {
  if (workoutDraft) return migrateWorkoutDraft(workoutDraft);
...
function renderTrain() {
  ...
  return `<div class="grid train-page">
    ...
    ${view==='history'?renderWorkoutHistoryView():renderActiveWorkout()}
  </div>`;
}
...
function discardWorkoutDraft() {
  ...
  const current=ensureWorkoutDraft();const {program,session}=getSession(current?.programId,current?.sessionId);
  workoutDraft=program&&session?buildWorkoutDraft(program,session):null;saveDraft();render();showToast('Workout discarded');
}
...
function saveWorkout() {
  ...
  saveState();
  const {program,session}=getSession(draft.programId,draft.sessionId);
  workoutDraft=program&&session?buildWorkoutDraft(program,session):null;saveDraft();closeModal();render();
