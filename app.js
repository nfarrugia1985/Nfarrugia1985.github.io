...
const PREP_WORDS=new Set(['fresh','finely','roughly','thinly','thickly','chopped','diced','sliced','minced','grated','crushed','peeled','trimmed','washed','drained','rinsed','optional','divided','plus','extra','for','serving','garnishing','garnish','to','taste','about','approximately','heaped','level','packed','large','medium','small','bigger','better','low','sodium','reduced','fat','free','organic','seasoned','ground','whole','room','temperature']);
...
function normaliseFoodName(value,{loose=false}={}){
  const words=String(value||'').toLowerCase().replace(/&amp;/g,' and ').replace(/\blow[-\s]?sodium\b/g,' ').replace(/\bjuice\s+of\b/g,' juice ').replace(/[^a-z0-9\s]/g,' ').split(/\s+/).filter(Boolean).filter(w=>!PREP_WORDS.has(w));
...
function autoMatchRecipeIngredients(recipe){
  normaliseRecipeRecord(recipe);recipe.ingredients.forEach(ing=>{
    ...
    const matches=localFoodMatches(ing.name||ing.sourceLine,4);
...
async function findUnmatchedIngredientsOnline(){
  if(!recipeDraft)return;const targets=recipeDraft.ingredients.map((ing,index)=>({ing,index})).filter(x=>!x.ing.ingredientId&&x.ing.notes!=='Section heading');
  ...
  try{
    for(let n=0;n<Math.min(targets.length,15);n++){
      ...
      const q=referenceFoodQuery(ing.name||ing.sourceLine);const result=await jsonpRequest({action:'food-search',key:state.settings.syncKey,q,source:'all'},35000);const foods=result?.foods||[];
...
function recipeIngredientEditorHtml(ing,index){
  const status=ing.matchStatus||'unmatched',statusClass=status==='confirmed'?'good':status==='likely'?'warn':['needs-measure','missing-amount'].includes(status)?'warn':'danger';const statusText=status==='needs-measure'?'Matched · unit conversion needed':status==='missing-amount'?'Matched · amount needed':status==='unmatched'?'Unmatched':status[0].toUpperCase()+status.slice(1);
  return `<div class="row recipe-ingredient-row">
    ...
    ${ing.matchStatus==='needs-measure'?'<div class="tiny muted">The food was matched, but this household unit needs a gram conversion.</div>':ing.matchStatus==='missing-amount'?'<div class="tiny muted">The food was matched. Enter an amount only if you want it included in nutrition totals.</div>':''}
  </div>`;
}
...
async function extractRecipesFromEpub(file){
  ...
  const manifest={};byTag('item').forEach(item=>manifest[item.getAttribute('id')]={href:item.getAttribute('href'),type:item.getAttribute('media-type'),properties:item.getAttribute('properties')||''});
  ...
  for(const href of hrefs){
    ...
    const navLabel=epubTitleCandidate(doc,path);
    for(const data of extractRecipesFromHtmlDocument(doc,navLabel)){
      const key=`${normaliseFoodName(data.name)}|${(data.ingredients||[]).slice(0,3).map(normaliseFoodName).join('|')}`;if(!data.name||data.ingredients.length<2||seen.has(key))continue;seen.add(key);recipes.push({...data,sourceName:title,bookTitle:title,author});if(recipes.length>=500)break;
    }
...
function epubTitleCandidate(doc,path=''){
  const title=[...doc.querySelectorAll('h1,h2,[class*="title" i],[id*="title" i]')].map(x=>x.textContent.replace(/\s+/g,' ').trim()).find(x=>x.length>2&&x.length<140&&!/^(contents?|ingredients?|method|directions?|instructions?)$/i.test(x));
  return title||String(doc.title||'').trim()||String(path).split('/').pop().replace(/\.[^.]+$/,'').replace(/[_-]+/g,' ');
}
...
function extractSemanticRecipe(doc,titleHint){
  ...
  if(ingredients.length>=2&&instructions.length){return {name:epubTitleCandidate(doc,titleHint),ingredients:[...new Set(ingredients)],instructions:[...new Set(instructions)].join('\n'),servings:parseEpubServing(doc),nutrition:{}};}return null;
}
...
function extractRecipesFromHtmlDocument(doc,titleHint=''){
  ...
  const semantic=extractSemanticRecipe(doc,titleHint);if(semantic)out.push(semantic);
  ...
  ingredientIndexes.forEach((i,idx)=>{
    let title='';for(let j=i-1;j>=0;j--){const t=blocks[j].text;if((blocks[j].heading||/^h[1-6]$/.test(blocks[j].tag))&&!isIngredientHeading(t)&&!isInstructionHeading(t)&&t.length<160){title=t;break;}}
    title=title||epubTitleCandidate(doc,titleHint);let methodIndex=-1;for(let j=i+1;j<blocks.length;j++){if(isInstructionHeading(blocks[j].text)){methodIndex=j;break;}if(idx+1<ingredientIndexes.length&&j>=ingredientIndexes[idx+1])break;}
