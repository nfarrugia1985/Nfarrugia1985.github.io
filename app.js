...
function migrateState(input) {
  ...
  next.programs.forEach((p, pi) => {
    ...
    const shippedProgram = ['UL4','RFL2','GVS_EXTRA'].includes(p.id);
...
function isBuiltInProgram(program) {
  ...
  return program.builtIn === true || ['UL4','RFL2','GVS_EXTRA'].includes(program.id);
}
...
function renderSettings() {
  ...
  return `<div class="grid">
    ...
    <div class="card" id="syncSettingsCard">
      ...
      <div class="notice ${['conflict','cloud-newer','verification-needed','error'].includes(syncState)?'warn':''}">The app saves on this phone first. v2.5.2 distinguishes the last cloud revision actually merged from a newer revision merely observed, so a status check cannot authorise an unsafe overwrite.</div>
